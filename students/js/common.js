...
function name_format(word) {
	...
	if(word.search("-")==-1) {
		...
		lereste = getTextBetween(word, 1, word.search("-")).toLowerCase()+getTextBetween(word, word.search("-")+1, word.length).substring(0,1).toUpperCase()+getTextBetween(word, word.search("-")+1, word.length).substring(1,word.length).toLowerCase();	
	}
