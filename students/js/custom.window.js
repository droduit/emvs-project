function openWindow(url, data, taille, title) {
	...
	if(taille=="" || typeof(taille)=='undefined') { taille = 600; }
	$("#window").css({"width":taille+"px","margin-left":-(taille/2+10)});
	$("#window .container").css("width",(taille-23)+"px");
	$("a").css("cursor","wait");
	
	if(title=="" || typeof(title)=='undefined') { titre = ""; } else { titre = title; }
	$("#window .title").html(titre);
	
	setTimeout(function() {$("#window").fadeIn("fast"); }, 200);
	...
	if(url.substring(0,wordLink.length)==wordLink) {
		$("#window .container").html('<iframe src="'+url+'" frameborder="0" width="100%" height="100%" scrolling="auto"></iframe>');
		
		$("#window").animate({marginLeft:-(width/2)+"px", marginTop:"10px"},200);	
		setTimeout(function(){
			$("#window .container, #window").animate({width:width+"px", height:height+"px"},200);
			$("#window .container").animate({ width:(width-23)+"px", height:(height-18)+"px"},200);
		},200);
	} else {
...
function closeWindow(no) {
	$("#window").fadeOut(250);
	setTimeout(function(){$("#window .container").html("");},300);
}
//***********************************************************************************************************
...
function resizeWindow(width,height) {
	...
	$("#window .container").animate({ width:(width-23)+"px", height:(height-18)+"px"},100);
}