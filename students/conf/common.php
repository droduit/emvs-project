...
function getNextId() {
	...
	$maxID = $bdd->query("SELECT MAX(PKNoMembre) as maxi from students_login");
	$maxID2 = $maxID->fetch();
...
function getID($id_crypte) {
	...
	$id = 0;
	$slctMembres = $bdd->query("SELECT PKNoMembre FROM students_login WHERE SHA1(MD5(CONCAT(PKNoMembre, '--..--', email, '', key_crypte))) = '" . $id_crypte . "' ORDER BY PKNoMembre");
...
function securise($variable) {
	...
	$securite = htmlspecialchars(addslashes(trim($variable)));
	return $securite;
}
function get_ip() {
...
function ImagesFromDirectory($Directory, $selection = "", $event_click = "", $type = "img", $loader = 1, $sous_jacent = 1) {
	...
	if ($type == "img") {
		...
		$query = $bdd->query("SELECT * FROM tbl_history_documents WHERE FKNoProjectHistory=" . $Directory);
	}
